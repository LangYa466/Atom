package org.atom.api.font.impl;
...
public class UFontRenderer extends FontRenderer implements Wrapper {
    ...
    public UFontRenderer(String name, int size) {
        ...
        try {
            ...
            ChatUtil.logger.error("Error loading font {}", name);
    ...
    public UFontRenderer(File inputFont, int size) {
        super(
                mc.gameSettings,
                new ResourceLocation("textures/font/ascii.png"),
                mc.getTextureManager(),
        ...
        Font font;
        try {
            InputStream is = Files.newInputStream(inputFont.toPath());
            font = Font.createFont(0, is);
            font = font.deriveFont(Font.PLAIN, size);
        } catch (Exception ex) {
            ChatUtil.logger.error("Error loading font {}", inputFont.getName());
            font = new Font("Arial", Font.PLAIN, size);
        }
    ...
    public void drawCenteredStringWithShadow(String text, float x, float y, int color) {
      drawString(text, x - stringCache.getStringWidth(text) / 2f, y, color, true);
    }
